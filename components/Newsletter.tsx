...
export default function Newsletter({ dict }: Props) {
  ...
  return (
    <div className="p-6 sm:p-8 rounded-md bg-stone-900 border border-stone-800">
      ...
      {submitted ? (
        ...
        <form onSubmit={onSubmit} className="mt-5 flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-stone-800 border border-stone-700 rounded-md text-bone placeholder:text-stone-500 focus:outline-none focus:border-stone-400 transition-colors text-sm"
          />
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-2.5 bg-bone text-ink rounded-md text-sm font-medium hover:bg-cream transition-colors disabled:opacity-60"
          >
